...
static std::string MinitableName(Context<Descriptor> msg) {
  return absl::StrReplaceAll(msg.desc().full_name(),
                             {{"_", "_0"}, {".", "__"}}) +
         "_msg_init";
}
...
impl<'a> $pb$::SettableValue<$Msg$> for $Msg$View<'a> {
  fn set_on(self, _private: $pb$::__internal::Private, mutator: $pb$::Mut<$Msg$>) {
    unsafe { $pbr$::upb_Message_DeepCopy(mutator.inner.msg(), self.msg, &$minitable$, mutator.inner.arena()) };
  }
}
                )rs");
             }
...
impl $pbi$::RepeatedMessageOps for $Msg$ {
  ...
  unsafe fn get(f: $pbr$::RepeatedField<'_, Self>, index: usize) -> Self::View<'_> {
    $Msg$View::new($pbi$::Private, unsafe{ $repeated_get_thunk$(f.raw(), index) })
  }
  unsafe fn get_mut(f: $pbr$::RepeatedField<'_, Self>, index: usize) -> Self::Mut<'_> {
    let msg = unsafe { $repeated_get_thunk$(f.raw(), index) };
    let mutator = $pbr$::MutatorMessageRef::new_raw($pbi$::Private, msg);
    $Msg$Mut{inner: mutator}
  }
  fn push_default(f: $pbr$::RepeatedField<'_, Self>) -> Self::Mut<'_> {
...
impl $pbi$::RepeatedMessageOps for $Msg$ {
  ...
  unsafe fn get(f: $pbr$::RepeatedField<'_, Self>, index: usize) -> Self::View<'_> {
    $Msg$View::new($pbi$::Private, unsafe{ $pbr$::upb_Array_Get(f.raw(), index).msg_val })
  }
  unsafe fn get_mut(f: $pbr$::RepeatedField<'_, Self>, index: usize) -> Self::Mut<'_> {
    let msg = unsafe { $pbr$::upb_Array_Get(f.raw(), index).msg_val };
    let mutator = $pbr$::MutatorMessageRef::new_ref($pbi$::Private, msg, &f.inner().arena);
    $Msg$Mut{inner: mutator}
  }
  fn push_default(f: $pbr$::RepeatedField<'_, Self>) -> Self::Mut<'_> {
    let msg = unsafe { $new_thunk$(f.inner().arena.raw()) };
    unsafe { $pbr$::upb_Array_Append(f.raw(), $pbr$::upb_MessageValue{ msg_val: msg }, f.inner().arena.raw()) };
    let mutator = $pbr$::MutatorMessageRef::new_ref($pbi$::Private, msg, &f.inner().arena);
    $Msg$Mut{inner: mutator}
  }
  fn clear(f: $pbr$::RepeatedField<'_, Self>) {
...
void GenerateThunksCc(Context<Descriptor> msg) {
  ...
  msg.Emit(
      ...
      R"cc(
        ...
        void $copy_from_thunk$($QualifiedMsg$* dst, $QualifiedMsg$& src) {
          dst->CopyFrom(src);
        }
